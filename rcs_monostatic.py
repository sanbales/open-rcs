...
def rcs_monostatic(params_entrys: list, coordinatesData: list) -> tuple[str, list, list]:
    ...
    # pattern loop
    Area, alpha, beta, N, d, ip, it = calculate_values(
        pstart, pstop, delp, tstart, tstop, delt, ntria, rad
    )
            ...
            for m in range(ntria):  # test to see if front face is illuminated
                ...
                if iflag == 0:
                    if (ilum[m] == 1 and ndotk >= 1e-5) or ilum[m] == 0:
                        ...
                        Jx2 = -Et2 * math.cos(phi2) * para + Ep2 * math.sin(phi2) * perp * math.cos(
                            th2
                        )  # math.cos(th2) removed
                        Jy2 = -Et2 * math.sin(phi2) * para - Ep2 * math.cos(phi2) * perp * math.cos(
                            th2
                        )  # math.cos(th2) removed
    ...
    param = plotParameters(
        "Monostatic", freq, wave, corr, delstd, pol, ntria, pstart, pstop, delp, tstart, tstop, delt
    )
