...
Open RCS provides monostatic and bistatic solvers based on a high-frequency **Physical Optics (PO)** approximation derived from Jenn's POFACETS. In this method, each illuminated facet is processed as an isolated scattering surface and its contribution is summed into the final RCS result. The current implementation does **not** model multiple-bounce interactions between facets, cavity effects, or edge-diffraction mechanisms. In practical terms, this means Open RCS can approximate the direct return from a shape, but it does **not** capture the strong multi-bounce response of targets such as corner reflectors. Furthermore, **TMz** and **TEz** polarizations are represented using complex data types in Python 3.
