...
class App(customtkinter.CTk):
    ...
    def define_sidebars(self):
        ...
        self.logo_label = customtkinter.CTkLabel(
            self.sidebar_frame, text="Open RCS", font=customtkinter.CTkFont(size=20, weight="bold")
        )
        self.logo_label.grid(row=1, column=0, padx=20, pady=(10, 10))
        # self.organization = customtkinter.CTkLabel(self.sidebar_frame, text="CIGE - Electronic Warfare Instruction Center", anchor="w")
        # self.organization.grid(row=2, column=0, padx=20, pady=(0, 10),sticky="s")
        self.appearance_mode_label = customtkinter.CTkLabel(
            self.sidebar_frame, text="Modo de Aparência:", anchor="s"
        )
    ...
    def define_description_frame_and_tabview(self):
        ...
        self.descriptionFrame.grid(
            row=0, column=1, columnspan=1, padx=(20, 0), pady=(20, 0), sticky="new"
        )
    ...
    def define_monostatic_inputs(self):
        self.monomodel_text = "\n⬆\nUpload Modelo (.stl)\n"
        self.monotext = customtkinter.CTkLabel(
            self.tabview.tab("Monoestático"),
            text="Insira os dados para o cálculo monoestático da RCS estimada",
        )
        self.monotext.grid(row=0, column=0, columnspan=3, padx=5, pady=(5, 5), sticky="ew")
        self.monomodel = customtkinter.CTkButton(
            self.tabview.tab("Monoestático"),
            text=self.monomodel_text,
            command=self.upload,
            fg_color=ThemeManager.theme["CTkEntry"]["fg_color"],
            text_color=ThemeManager.theme["CTkEntry"]["placeholder_text_color"],
        )
        ...
        self.monofreq = customtkinter.CTkEntry(
            self.tabview.tab("Monoestático"), placeholder_text="Frequência (GHz)"
        )
        self.monofreq.grid(row=1, column=0, padx=5, pady=(5, 5))
        self.monocorr = customtkinter.CTkEntry(
            self.tabview.tab("Monoestático"), placeholder_text="Distância (m)"
        )
        self.monocorr.grid(row=1, column=1, padx=5, pady=(5, 5))
        self.monodelstd = customtkinter.CTkEntry(
            self.tabview.tab("Monoestático"), placeholder_text="Desvio Padrão (m)"
        )
        self.monodelstd.grid(row=1, column=2, padx=5, pady=(5, 5))
        self.monoipol = customtkinter.CTkOptionMenu(
            self.tabview.tab("Monoestático"),
            values=["TM-Z", "TE-Z"],
            fg_color=ThemeManager.theme["CTkEntry"]["fg_color"],
            text_color=ThemeManager.theme["CTkEntry"]["placeholder_text_color"],
        ...
        self.monorest = customtkinter.CTkOptionMenu(
            self.tabview.tab("Monoestático"),
            values=["Material Específico", "Condutor Perfeito"],
            fg_color=ThemeManager.theme["CTkEntry"]["fg_color"],
            text_color=ThemeManager.theme["CTkEntry"]["placeholder_text_color"],
        )
        self.monorest.grid(row=3, column=0, padx=5, pady=(5, 5))
        self.monorest.set("Resistividade")
        self.monopstart = customtkinter.CTkEntry(
            self.tabview.tab("Monoestático"), placeholder_text="Phi Inicial (º)"
        )
        self.monopstart.grid(row=2, column=1, padx=5, pady=(5, 5))
        self.monopstop = customtkinter.CTkEntry(
            self.tabview.tab("Monoestático"), placeholder_text="Phi Final (º)"
        )
        self.monopstop.grid(row=3, column=1, padx=5, pady=(5, 5))
        self.monodelp = customtkinter.CTkEntry(
            self.tabview.tab("Monoestático"), placeholder_text="Passo Phi (º)"
        )
        self.monodelp.grid(row=4, column=1, padx=5, pady=(5, 5))
        self.monotstart = customtkinter.CTkEntry(
            self.tabview.tab("Monoestático"), placeholder_text="Teta Inicial (º)"
        )
        self.monotstart.grid(row=2, column=2, padx=5, pady=(5, 5))
        self.monotstop = customtkinter.CTkEntry(
            self.tabview.tab("Monoestático"), placeholder_text="Teta Final (º)"
        )
        self.monotstop.grid(row=3, column=2, padx=5, pady=(5, 5))
        self.monodelt = customtkinter.CTkEntry(
            self.tabview.tab("Monoestático"), placeholder_text="Passo Teta (º)"
        )
        self.monodelt.grid(row=4, column=2, padx=5, pady=(5, 5))
        self.monoresult = customtkinter.CTkButton(
            self.tabview.tab("Monoestático"),
            text="Gerar Resultados",
            command=lambda: self.generate_and_show_results_event("monostatic", "interface"),
        )
        self.monoresult.grid(row=6, column=1, padx=5, pady=(40, 0), sticky="nsew")
        self.monoresultfile = customtkinter.CTkButton(
            self.tabview.tab("Monoestático"),
            text="Gerar Resultados do Input File",
            command=lambda: self.generate_and_show_results_event("monostatic", "inputFile"),
            fg_color=ThemeManager.theme["CTkEntry"]["fg_color"],
            text_color=ThemeManager.theme["CTkEntry"]["placeholder_text_color"],
        )
        self.monoresultfile.grid(row=7, column=0, columnspan=3, padx=5, pady=(10, 0))
        self.monoerror = customtkinter.CTkLabel(
            self.tabview.tab("Monoestático"),
            text="",
    ...
    def define_bistatic_inputs(self):
        self.bitext = customtkinter.CTkLabel(
            self.tabview.tab("Biestático"),
            text="Insira os dados para o cálculo biestático da RCS estimada",
        )
        self.bitext.grid(row=0, column=0, columnspan=3, padx=5, pady=(5, 5), sticky="ew")
        self.bimodel = customtkinter.CTkButton(
            self.tabview.tab("Biestático"),
            text="\n⬆\nUpload Modelo (.stl)\n",
            command=self.upload,
            fg_color=ThemeManager.theme["CTkEntry"]["fg_color"],
        ...
        self.bifreq = customtkinter.CTkEntry(
            self.tabview.tab("Biestático"), placeholder_text="Frequência (GHz)"
        )
        self.bifreq.grid(row=1, column=0, padx=5, pady=(5, 5))
        self.bicorr = customtkinter.CTkEntry(
            self.tabview.tab("Biestático"), placeholder_text="Distância (m)"
        )
        self.bicorr.grid(row=1, column=1, padx=5, pady=(5, 5))
        self.bidelstd = customtkinter.CTkEntry(
            self.tabview.tab("Biestático"), placeholder_text="Desvio Padrão (m)"
        )
        self.bidelstd.grid(row=1, column=2, padx=5, pady=(5, 5))
        self.biipol = customtkinter.CTkOptionMenu(
            self.tabview.tab("Biestático"),
        ...
        self.birest = customtkinter.CTkOptionMenu(
            self.tabview.tab("Biestático"),
            values=["Material Específico", "Condutor Perfeito"],
            fg_color=ThemeManager.theme["CTkEntry"]["fg_color"],
            text_color=ThemeManager.theme["CTkEntry"]["placeholder_text_color"],
        ...
        self.biphi = customtkinter.CTkEntry(
            self.tabview.tab("Biestático"), placeholder_text="Phi Incidente (º)"
        )
        self.biphi.grid(row=2, column=1, padx=5, pady=(5, 5))
        self.bitheta = customtkinter.CTkEntry(
            self.tabview.tab("Biestático"), placeholder_text="Teta Incidente (º)"
        )
        self.bitheta.grid(row=2, column=2, padx=5, pady=(5, 5))
        self.bipstart = customtkinter.CTkEntry(
            self.tabview.tab("Biestático"), placeholder_text="Phi Inicial (º)"
        )
        self.bipstart.grid(row=3, column=1, padx=5, pady=(5, 5))
        self.bipstop = customtkinter.CTkEntry(
            self.tabview.tab("Biestático"), placeholder_text="Phi Final (º)"
        )
        self.bipstop.grid(row=4, column=1, padx=5, pady=(5, 5))
        self.bidelp = customtkinter.CTkEntry(
            self.tabview.tab("Biestático"), placeholder_text="Passo Phi (º)"
        )
        self.bidelp.grid(row=5, column=1, padx=5, pady=(5, 5))
        self.bitstart = customtkinter.CTkEntry(
            self.tabview.tab("Biestático"), placeholder_text="Teta Inicial (º)"
        )
        self.bitstart.grid(row=3, column=2, padx=5, pady=(5, 5))
        self.bitstop = customtkinter.CTkEntry(
            self.tabview.tab("Biestático"), placeholder_text="Teta Final (º)"
        )
        self.bitstop.grid(row=4, column=2, padx=5, pady=(5, 5))
        self.bidelt = customtkinter.CTkEntry(
            self.tabview.tab("Biestático"), placeholder_text="Passo Teta (º)"
        )
        self.bidelt.grid(row=5, column=2, padx=5, pady=(5, 5))
        self.biresult = customtkinter.CTkButton(
            self.tabview.tab("Biestático"),
            text="Gerar Resultados",
            command=lambda: self.generate_and_show_results_event("bistatic", "interface"),
        )
        self.biresult.grid(row=7, column=1, padx=5, pady=(40, 0), sticky="nsew")
        self.biresultfile = customtkinter.CTkButton(
            self.tabview.tab("Biestático"),
            text="Gerar Resultados do Input File",
            command=lambda: self.generate_and_show_results_event("bistatic", "inputFile"),
            fg_color=ThemeManager.theme["CTkEntry"]["fg_color"],
            text_color=ThemeManager.theme["CTkEntry"]["placeholder_text_color"],
        )
    ...
    def define_results_frame(self):
        ...
        self.results_frame.grid(
            row=0, column=2, rowspan=2, columnspan=2, padx=(20, 20), pady=(20, 0), sticky="nsew"
        )
        self.results_frame.grid_columnconfigure((0, 1, 2), weight=1)
        self.label_results = customtkinter.CTkLabel(
            self.results_frame,
            text="Resultados",
        ...
        self.cancel = customtkinter.CTkButton(
            self.results_frame,
            text="Cancelar Carregamento",
            command=self.end_generate_attempt,
            fg_color=ThemeManager.theme["CTkEntry"]["fg_color"],
            text_color=ThemeManager.theme["CTkEntry"]["placeholder_text_color"],
        )
    ...
    def define_material_frame(self):
        ...
        self.material_text = customtkinter.CTkLabel(
            self.material_window, text="Selecione o Tipo de Material"
        )
        self.material_text.grid(row=0, column=0, columnspan=2, padx=10, pady=10)

        self.material_type = customtkinter.CTkOptionMenu(
            self.material_window,
            values=[
                "Material Transparente",
                "PEC",
                "Composito",
                "Camada de Composito em PEC",
                "Multiplas Camadas",
                "Multiplas Camadas em PEC",
            ],
            fg_color=ThemeManager.theme["CTkEntry"]["fg_color"],
            text_color=ThemeManager.theme["CTkEntry"]["placeholder_text_color"],
            command=self.on_select_material_type,
        )
        ...
        self.relperm_entry = customtkinter.CTkEntry(
            self.material_window, placeholder_text="Permissividade Rel."
        )
        self.relperm_entry.grid(row=6, column=0, padx=5, pady=(5, 5))

        self.losstang_entry = customtkinter.CTkEntry(
            self.material_window, placeholder_text="Tang. de Perdas"
        )
        ...
        self.real_entry = customtkinter.CTkEntry(
            self.material_window, placeholder_text="Parte Real"
        )
        self.real_entry.grid(row=9, column=0, padx=5, pady=(5, 5))

        self.imag_entry = customtkinter.CTkEntry(
            self.material_window, placeholder_text="Parte Imaginária"
        )
        ...
        self.thick_entry = customtkinter.CTkEntry(
            self.material_window, placeholder_text="Espessura"
        )
        self.thick_entry.grid(row=11, column=0, columnspan=2, padx=5, pady=(5, 5))

        self.button_addlayer = customtkinter.CTkButton(
            self.material_window,
            text="Adicionar Camada",
            command=lambda: self.add_new_layer_event(),
        )

        self.button_removelayer = customtkinter.CTkButton(
            self.material_window,
            text="Remover Última Camada",
            command=lambda: self.remove_last_layer(),
        )

        self.material_message = customtkinter.CTkLabel(
            self.material_window, text="", font=customtkinter.CTkFont(size=10, weight="bold")
        )
        self.material_message.grid(row=13, column=0, columnspan=2, padx=5, pady=(5, 5))

        self.button_openconfig = customtkinter.CTkButton(
            self.material_window,
            text="Calcular com Arquivo",
            command=lambda: self.initiate_thread_for_function(
                self.define_material_properties_list_from_material_file
            ),
        )
        self.button_openconfig.grid(row=14, column=1, padx=5, pady=(5, 5))

        self.button_actualconfig = customtkinter.CTkButton(
            self.material_window,
            text="Ver Camadas Atuais",
            command=lambda: self.show_actual_material_config_event(),
        )
        self.button_actualconfig.grid(row=14, column=0, padx=5, pady=(5, 5))

        self.button_saveconfig = customtkinter.CTkButton(
            self.material_window,
            text="Salvar",
            command=lambda: self.save_current_material_properties(),
        )
        self.button_saveconfig.grid(row=15, column=0, padx=5, pady=(5, 5))

        self.button_continue = customtkinter.CTkButton(
            self.material_window,
            text="Calcular RCS",
            command=lambda: self.initiate_thread_for_function(
                self.run_write_matrl_and_calculate_rcs
            ),
        )
    ...
    def define_actual_material_frame(self):
        ...
        self.scrollbar = customtkinter.CTkScrollbar(
            self.material_actual_configuration, orientation="vertical", command=self.canvas.yview
        )
        ...
        self.slider = customtkinter.CTkSlider(
            self.inner_frame,
            from_=self.facetBeginIndex,
            to=self.facetEndIndex,
            command=self.on_slidebar_change,
        )
    ...
    def getParamsFromInterface(self) -> list:
        ...
        def convert_reisivity(rest):
            if rest == "Condutor Perfeito":
                return 0
            elif rest == "Material Específico":
                return 1
    ...
    def get_entrys_from_material_interface(self):
        try:
            ...
            self.type = self.material_type.get()
            self.thickness = float(self.thick_entry.get())
            self.RelPermittivity = float(self.relperm_entry.get())
            self.lossTangent = float(self.losstang_entry.get())
            self.RelaPermeabilityReal = float(self.real_entry.get())
            self.RelaPermeabilityImaginary = float(self.imag_entry.get())
            return True
    ...
    def run_write_matrl_and_calculate_rcs(self):
        if self.get_entrys_from_material_interface():
            ...
            if (
                self.type == "Multiplas Camadas" or self.type == "Multiplas Camadas em PEC"
            ) and self.material_properties_list != []:
                self.material_window.withdraw()
                save_list_in_file(self.material_properties_list, "matrl.txt")
                self.calculate_and_show_rcs_results()
    ...
    def show_actual_material_config_event(self):
        if (
            ...
            or self.type == "Multiplas Camadas"
            or self.type == "Multiplas Camadas em PEC"
        ) and len(self.material_properties_list) != 0:
    ...
    def define_material_properties_list_from_material_file(self):
        ...
        materialFile = askopenfile(
            title="Selecionar um arquivo", filetypes=[("Text files", "*.txt")]
        )
    ...
    def save_current_material_properties(self):
        ...
        elif (self.type == "Multiplas Camadas" or self.type == "Multiplas Camadas em PEC") and len(
    ...
    def add_current_layer(self):
        layerProperties = [
            self.RelPermittivity,
            self.lossTangent,
            self.RelaPermeabilityReal,
            self.RelaPermeabilityImaginary,
            self.thickness,
        ]
    ...
    def get_facets_indexs(self):
        ffacet_value = self.ffacet_entry.get()
        lfacet_value = self.lfacet_entry.get()
    ...
    def calculate_and_show_rcs_results(self):
        ...
        if (
            self.plotpath == "Number of entrys in matrl diferent from number of facets."
            or self.plotpath == "Matrl file not found."
        ):
    ...
    def initiate_process(self, method):
        ...
        if method == "monostatic":
            self.process = multiprocessing.Process(
                target=run_rcs_monostatic,
                args=(self.simulationParamsList, self.coordinatesData, self.result_queue),
            )
        elif self.method == "bistatic":
            self.process = multiprocessing.Process(
                target=run_rcs_bistatic,
                args=(self.simulationParamsList, self.coordinatesData, self.result_queue),
            )
    ...
    def results_window(self):
        ...
        self.plottext = customtkinter.CTkLabel(
            self.results_frame, text="Seção Reta Radar do Alvo Carregado"
        )
        ...
        # Figure - ensure the image is square
        ...
        self.figtext = customtkinter.CTkLabel(
            self.results_frame, text="Modelo Triangular do Alvo Carregado (.stl)"
        )
        ...
        self.saveplot = customtkinter.CTkButton(
            self.results_frame, text="⬇ Download Gráfico", command=self.save_plot, width=300
        )
        self.saveplot.grid(row=3, column=1, columnspan=2, padx=5, pady=(25, 5))
        self.savefile = customtkinter.CTkButton(
            self.results_frame,
            text="⬇ Download Modelo Triangular",
            command=self.save_fig,
            width=300,
        )
        self.savefile.grid(row=4, column=1, columnspan=2, padx=5, pady=(5, 5))
        self.savefig = customtkinter.CTkButton(
            self.results_frame,
            text="⬇ Download Arquivo de Dados",
    ...
    def upload(self):
        file = askopenfile(title="Selecionar um arquivo", filetypes=[("Arquivos STL", "*.stl")])
        if file:
    ...
    def load_header_of_material_table(self):
        ...
        for col_index, col_name in enumerate(columns):
            header_label = customtkinter.CTkLabel(
                self.table_inner_frame, text=col_name, font=customtkinter.CTkFont(weight="bold")
            )
    ...
    def redefine_table_inner_frame(self, size):
        ...
        self.material_actual_configuration.geometry(
            f"{self.table_width + 30}x{row_height * ideal_height + 50}"
        )
    ...
    def on_slidebar_change(self, new_value=1):
        self.slider_value_label.configure(
            text=f"Faceta Selecionada: {int(new_value)}",
            font=customtkinter.CTkFont(size=13, weight="bold"),
        )
    ...
    def on_save(self):
        messagebox.showinfo(
            "Arquivo Salvo", "Arquivo salvo na pasta results do diretório do OpenRCS"
        )
