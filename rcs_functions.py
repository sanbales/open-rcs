...
def setFontOption(
    fontSize=SMALL_SIZE,
    axesTitle=MEDIUM_SIZE,
    axesLabel=SMALL_SIZE,
    xtickLabel=SMALL_SIZE,
    ytickLabel=SMALL_SIZE,
    legendSize=SMALL_SIZE,
    figureTitle=BIGGER_SIZE,
):
...
def getParamsFromFile(method):
    ...
    params = open(input_data_file)
    param_list = []
    for row in params:
        row = row.strip("\n")
        if not row.startswith("#"):
            if row.isnumeric():
                param_list.append(float(row))
            else:
...
def direction_cosines(alpha, beta, D0, m):
    T1 = np.array(
        [
            [math.cos(alpha[m]), math.sin(alpha[m]), 0],
    ...
    T2 = np.array(
        [
            [math.cos(beta[m]), 0, -math.sin(beta[m])],
            [0, 1, 0],
...
def finalPlot(ip, it, phi, wave, theta, Lmin, Lmax, Sth, Sph, U, V, now, input_model, mode) -> str:
    if ip == 1:
        ...
        plt.title(
            f"target: {input_model}   solid: theta     dashed: phi     phi= {phi[0][0]}    wave (m): {round(wave, 6)}"
        )
    ...
    if it == 1:
        ...
        plt.suptitle(f"RCS Simulation IR Signature - {mode}")
        plt.title(
            f"target: {input_model}   solid: theta     dashed: phi     theta= {theta[0][0]}    wave (m): {round(wave, 6)}"
        )
...
def calculate_Ic(Dp, Dq, Do, N, Nt, Area, expDo, Co, Lt, DD, expDq, m, expDp):
    ...
    if abs(Dp) < Lt and abs(Dq) >= Lt:
        ...
        sic = 0.0
        for n in range(Nt + 1):
            sic = sic + (1j * Dp) ** n / math.factorial(n) * (
                -Co / (n + 1) + expDq * (Co * G(n, -Dq))
            )
    ...
    # special case 4
        ...
        specialcase = 0
        Ic = (
            2 * Area[m] * expDo * (expDp * Co / (Dp * DD) - expDq * Co / (Dq * DD) - Co / (Dp * Dq))
        )
...
def plotParameters(
    mode, freq, wave, corr, delstd, pol, ntria, pstart, pstop, delp, tstart, tstop, delt
):
    param = f"    Mode: {mode}\n\
...
def calculaCampos(
    ...
    Edif = (
        ...
        * Area[m]
        * (math.cos(th2) ** 2)
        * np.exp(-((corel * np.pi * math.sin(th2) / wave) ** 2))
    )
...
def calculateSth_Sph(cfac1, sumt, sump, sumdt, wave, Sth, Sph, i1, i2, sumdp):
    Sth[i1, i2] = 10 * np.log10(
        4 * np.pi * cfac1 * (np.abs(sumt) ** 2 + np.sqrt(1 - cfac1**2) * sumdt) / wave**2 + 1e-10
    )
    Sph[i1, i2] = 10 * np.log10(
        4 * np.pi * cfac1 * (np.abs(sump) ** 2 + np.sqrt(1 - cfac1**2) * sumdp) / wave**2 + 1e-10
    )
